   ...
   for ( var i = 0; i < 6; i++ ) {
      result += characters.charAt(Math.floor(Math.random() * charactersLength));
        ...
        $(".ms-quickbar-container .ms-todo-list").prepend(
          ...
          '<h6 class="ms-card-title"  id="idtitletotask'+result+'" >'+ $("#task-block").val() +'</h6> <button data-toggle="tooltip" data-placement="left" title="Add a Task to this block" class="ms-add-task-to-block ms-btn-icon float-right">' +
          '<i class="material-icons">add</i> </button> </div> <div class="ms-card-body"> <ul id="'+ result +'" class="ms-list ms-task-block"> </ul></div>' +
          '<div class="ms-card-footer clearfix"><a onclick="archiveNewTask(\''+result+'\');" class="text-disabled mr-2"> <i class="flaticon-archive"> </i> Archive </a><a href="#" class="text-disabled ms-delete-trigger float-right">' +
    ...
    function confirmTask(){
      $(".ms-todo-list").on('submit', '.ms-confirm-task-form', function(e){
        ...
        confirmBtn.addClass('');
        confirmBtn.text('');
        $(this).replaceWith('<input style=" border: 0;" readonly value="'+ taskInput.val() +'" ><button class="close"><i class="flaticon-trash ms-delete-trigger"> </i></button>');
